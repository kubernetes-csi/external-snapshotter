...
func TestGroupSnapshotFinalizer(t *testing.T) {
	tests := []controllerTest{
		{
			name: "7-1 - finalizer is added on dynamically provisioned group contents if the storage class specify a Deletion retain policy",
		...
		{
			name: "7-2 - finalizer is not added on dynamically provisioned group contents if the storage class specify the Retain retain policy",
			initialGroupSnapshots: newGroupSnapshotArray(
				"snap-1-1", "snapuid1-1", map[string]string{
					"app.kubernetes.io/name": "postgresql",
				},
				"", classSilver, "groupsnapcontent-snapuid1-1", &False, nil, nil, false, false, nil,
			),
			expectedGroupSnapshots: withGroupSnapshotFinalizers(
				newGroupSnapshotArray(
					"snap-1-1", "snapuid1-1", map[string]string{
			...
			initialGroupContents: newGroupSnapshotContentArray(
				"groupsnapcontent-snapuid1-1", "snapuid1-1", "snap-1-1", "snapshot-handle", classSilver, []string{
					"1-pv-handle6-1",
					"2-pv-handle6-1",
				}, "", deletionPolicy, nil, false, false,
			),
		...
		{
			name: "7-3 - dynamic group snapshot have been deleted, retention policy set to Delete - the finalizer will be removed",
