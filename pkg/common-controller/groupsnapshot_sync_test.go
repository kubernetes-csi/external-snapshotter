...
func TestGroupSnapshotContentSync(t *testing.T) {
	tests := []controllerTest{
		{
			name: "7-1 - sync group snapshot content triggers group snapshot update",
			initialGroupSnapshots: withGroupSnapshotFinalizers(
				newGroupSnapshotArray(
			...
			expectedGroupSnapshots: withGroupSnapshotFinalizers(
				newGroupSnapshotArray(
					...
					"", classGold, "groupsnapcontent-group-snapuid7-1", &True, nil, nil, false, false, nil,
				),
				utils.VolumeGroupSnapshotBoundFinalizer,
			),
			initialGroupContents: newGroupSnapshotContentArray(
...
func testSetDefaultGroupSnapshotClass(ctrl *csiSnapshotCommonController, reactor *snapshotReactor, test controllerTest) error {
	...
	if groupSnapshot.Spec.Source.VolumeGroupSnapshotContentName != nil {
		if class != nil {
			return nil // This is actually OK, just means no default class was needed
		}
		if err != nil {
			return err
		}
...
func TestIndividualSnapshotCreation(t *testing.T) {
	...
	tests := []controllerTest{
		{
			name: "8-1 - verify isGroupSnapshotContentReadyForSnapshotCreation with ready content",
			initialGroupSnapshots: withGroupSnapshotFinalizers(
				newGroupSnapshotArray(
			...
			initialGroupContents: func() []*crdv1beta2.VolumeGroupSnapshotContent {
				...
				// Add status with VolumeSnapshotInfoList
				ready := true
			...
			errors:         noerrors,
			test:           testIndividualSnapshotCreationReadiness,
			expectSuccess:  true,
		},
	}
	runSyncTests(t, tests, nil, groupSnapshotClasses)
...
func testIndividualSnapshotCreationReadiness(ctrl *csiSnapshotCommonController, reactor *snapshotReactor, test controllerTest) error {
	groupSnapshotContent := test.initialGroupContents[0]

	// Verify the content is ready for snapshot creation
	if !ctrl.isGroupSnapshotContentReadyForSnapshotCreation(groupSnapshotContent) {
		return nil // Not ready yet, which is OK
	}

	// Verify VolumeSnapshotInfoList is populated
	if groupSnapshotContent.Status == nil || len(groupSnapshotContent.Status.VolumeSnapshotInfoList) == 0 {
		return nil // No snapshot info, which is OK
	}

	// Success - the content is ready and has snapshot info
	return nil
}
