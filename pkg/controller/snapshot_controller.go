...
func (ctrl *csiSnapshotController) checkandUpdateBoundSnapshotStatusOperation(snapshot *crdv1.VolumeSnapshot, content *crdv1.VolumeSnapshotContent) (*crdv1.VolumeSnapshot, error) {
	...
	if err != nil {
		...
		glog.V(5).Infof("checkandUpdateBoundSnapshotStatusOperation: driver %s, snapshotId %s, creationTime %d, size %d, readyToUse %t", driverName, snapshotID, creationTime, size, readyToUse)
	}
...
func (ctrl *csiSnapshotController) createSnapshotOperation(snapshot *crdv1.VolumeSnapshot) (*crdv1.VolumeSnapshot, error) {
	...
	glog.V(5).Infof("Created snapshot: driver %s, snapshotId %s, creationTime %d, size %d, readyToUse %t", driverName, snapshotID, creationTime, size, readyToUse)

	var newSnapshot *crdv1.VolumeSnapshot
	// Update snapshot status with creationTime
	for i := 0; i < ctrl.createSnapshotContentRetryCount; i++ {
		glog.V(5).Infof("createSnapshot [%s]: trying to update snapshot creation creationTime", snapshotKey(snapshot))
		newSnapshot, err = ctrl.updateSnapshotStatus(snapshot, readyToUse, creationTime, size, false)
		if err == nil {
			break
		}
		glog.V(4).Infof("failed to update snapshot %s creation creationTime: %v", snapshotKey(snapshot), err)
	}
...
func (ctrl *csiSnapshotController) updateSnapshotStatus(snapshot *crdv1.VolumeSnapshot, readyToUse bool, createdAt time.Time, size int64, bound bool) (*crdv1.VolumeSnapshot, error) {
	glog.V(5).Infof("updating VolumeSnapshot[]%s, readyToUse %v, creationTime %v", snapshotKey(snapshot), readyToUse, createdAt)
