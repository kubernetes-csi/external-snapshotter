...
func (ctrl *csiSnapshotSideCarController) isDriverMatch(content *crdv1.VolumeSnapshotContent) bool {
	if content.Spec.Source.VolumeHandle == nil && content.Spec.Source.SnapshotHandle == nil {
		// Skip this snapshot content if it does not have a valide source
		return false
	}
	if content.Spec.Driver != ctrl.snapshotterName {
		// Skip this snapshot content if the driver does not match
		return false
	}
	snapshotClassName := content.Spec.SnapshotClassName
	if snapshotClassName != nil {
		if snapshotClass, err := ctrl.classLister.Get(*snapshotClassName); err == nil {
			if snapshotClass.Driver != ctrl.snapshotterName {
				return false
			}
		}
	}
...
func (ctrl *csiSnapshotSideCarController) verifySnapshotClass(snapshot *crdv1.VolumeSnapshot) (*crdv1.VolumeSnapshot, error) {
	...
	klog.V(5).Infof("verifySnapshotClass: VolumeSnapshotClass Snapshotter [%s] Snapshot Controller snapshotterName [%s]", class.Driver, ctrl.snapshotterName)
	if class.Driver != ctrl.snapshotterName {
		klog.V(4).Infof("verifySnapshotClass: Skipping VolumeSnapshot %s for snapshotter [%s] in VolumeSnapshotClass because it does not match with the snapshotter for controller [%s]", utils.SnapshotKey(snapshot), class.Driver, ctrl.snapshotterName)
...
func (ctrl *csiSnapshotSideCarController) deleteSnapshot(snapshot *crdv1.VolumeSnapshot) {
	...
	if snapshot.Status.BoundVolumeSnapshotContentName == nil || *snapshot.Status.BoundVolumeSnapshotContentName == "" {
		klog.V(5).Infof("snapshot %q is not bound to a content", utils.SnapshotKey(snapshot))
		return
	}
