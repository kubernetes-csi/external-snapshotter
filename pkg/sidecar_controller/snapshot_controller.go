...
func (ctrl *csiSnapshotSideCarController) setSnapshotAnnDynamicallyCreated(snapshot *crdv1.VolumeSnapshot) (*crdv1.VolumeSnapshot, error) {
	...
	updateSnapshot, err := ctrl.clientset.SnapshotV1alpha1().VolumeSnapshots(snapshotCopy.Namespace).Update(snapshotCopy)
	if err != nil {
...
func (ctrl *csiSnapshotSideCarController) getCreateSnapshotInput(snapshot *crdv1.VolumeSnapshot) (*crdv1.VolumeSnapshotClass, *v1.PersistentVolume, string, map[string]string, error) {
...
func (ctrl *csiSnapshotSideCarController) checkandUpdateBoundSnapshotStatusOperation(snapshot *crdv1.VolumeSnapshot, content *crdv1.VolumeSnapshotContent) (*crdv1.VolumeSnapshot, error) {
	...
	newSnapshot, err := ctrl.updateSnapshotStatus(snapshot, readyToUse, creationTime, size, utils.IsSnapshotBound(snapshot, content))
	if err != nil {
		return nil, err
	}
...
func (ctrl *csiSnapshotSideCarController) createSnapshotOperation(snapshot *crdv1.VolumeSnapshot) (*crdv1.VolumeSnapshot, error) {
	...
	for i := 0; i < ctrl.createSnapshotContentRetryCount; i++ {
		...
		if updateContent, err = ctrl.clientset.SnapshotV1alpha1().VolumeSnapshotContents().Create(snapshotContent); err == nil || apierrs.IsAlreadyExists(err) {
			// Save succeeded.
			if err != nil {
...
func (ctrl *csiSnapshotSideCarController) deleteSnapshotContentOperation(content *crdv1.VolumeSnapshotContent) error {
	...
	if snapshotClassName != nil {
		if snapshotClass, err := ctrl.classLister.Get(*snapshotClassName); err == nil {
			...
			snapshotterSecretRef, err := utils.GetSecretReference(snapshotClass.Parameters, content.Name, nil)
			if err != nil {
				return err
			}
			snapshotterCredentials, err = utils.GetCredentials(ctrl.client, snapshotterSecretRef)
...
func (ctrl *csiSnapshotSideCarController) updateSnapshotStatus(snapshot *crdv1.VolumeSnapshot, readyToUse bool, createdAt time.Time, size int64, bound bool) (*crdv1.VolumeSnapshot, error) {
