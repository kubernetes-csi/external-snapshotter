...
func NewCSISnapshotCommonController(
	...
	eventRecorder = broadcaster.NewRecorder(scheme.Scheme, v1.EventSource{Component: fmt.Sprintf("common-csi-snapshotter")})
...
func (ctrl *csiSnapshotCommonController) checkAndUpdateSnapshotClass(snapshot *crdv1.VolumeSnapshot) (*crdv1.VolumeSnapshot, error) {
	...
	klog.V(5).Infof("VolumeSnapshotClass [%s] Snapshotter [%s]", class.Name, class.Snapshotter)
